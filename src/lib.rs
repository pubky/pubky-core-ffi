...
pub fn get_signup_token(homeserver_pubky: String, admin_password: String) -> Vec<String> {
    let runtime = TOKIO_RUNTIME.clone();
    runtime.block_on(async {
        // Build the reqwest client and make the HTTP request directly
        ...
        let url_str = format!("https://{homeserver_pubky}/admin/generate_signup_token");
...
pub fn republish_homeserver(secret_key: String, homeserver: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        let signer = client.signer(keypair);
        match signer.pkdns().publish_homeserver_force(Some(&homeserver_public_key)).await
        {
            Ok(_) => {
...
pub fn sign_out(session_secret: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        let session = match pubky::PubkySession::import_secret(&session_secret, Some(http_client)).await {
            Ok(session) => session,
            Err(error) => return create_response_vector(true, format!("Failed to import session: {}", error)),
        };

        // Sign out
        match session.signout().await {
            Ok(_) => create_response_vector(false, "Sign out success".to_string()),
            Err((error, _)) => create_response_vector(true, format!("Failed to sign out: {}", error)),
        }
    })
}
...
// Note: put() now requires authentication via a session.
...
pub fn put(url: String, content: String, secret_key: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        let trimmed_url = url.trim_end_matches('/');
        // Extract the path from the URL (remove the protocol and host)
        let path = if let Some(path_start) = trimmed_url.find("/pub/") {
            &trimmed_url[path_start..]
        } else {
            return create_response_vector(true, "Invalid URL: must contain /pub/".to_string());
...
pub fn get(url: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        ...
        // Use public storage for unauthenticated reads
        ...
        match client.client().pkarr().resolve(&public_key).await {
            Some(signed_packet) => {
                ...
                let json_records: Vec<serde_json::Value> = all_records
                    .iter()
                    .map(|record| resource_record_to_json(record))
                    .collect();
                ...
                let json_obj = json!({
                    "signed_packet": hex,
                    "public_key": base64_engine.encode(public_key),
                    "signature": base64_engine.encode(signature),
                    "timestamp": timestamp,
                    "last_seen": signed_packet.last_seen(),
                    "dns_packet": base64_engine.encode(dns_packet),
...
pub fn list(url: String) -> Vec<String> {
    ...
    runtime.block_on(async {
        let client = get_pubky_client();
        // Ensure URL ends with / for directory listings
        let mut trimmed_url = url.trim_end_matches('/').to_string();
        trimmed_url.push('/');
        ...
        let list_builder = match public_storage.list(&trimmed_url) {
            Ok(list) => list,
            Err(error) => {
                return create_response_vector(true, format!("Failed to create list builder: {}", error))
            }
        };
        ...
        let resource_strings: Vec<String> = resources.iter().map(|r| r.to_pubky_url()).collect();
        let json_string = match serde_json::to_string(&resource_strings) {
            Ok(json) => json,
            Err(error) => {
...
pub fn create_recovery_file(secret_key: String, passphrase: String) -> Vec<String> {
    ...
    let recovery_file = base64_engine.encode(&recovery_file_bytes);
...
pub fn decrypt_recovery_file(recovery_file: String, passphrase: String) -> Vec<String> {
    ...
    let recovery_file_bytes = match base64_engine.decode(&recovery_file) {
        Ok(bytes) => bytes,
        Err(error) => {
            return create_response_vector(
                true,
